...
import { readApi, money } from "../shared/api";
...
export function Dashboard() {
  const loader = useCallback((signal: AbortSignal) => readApi<Data>("/api/admin/dashboard", signal), []);
  const { data, loading, error, retry } = useResource(loader);
  if (loading && !data) return <><Typography.Title level={3}>仪表盘</Typography.Title><DashboardSkeleton /></>;
  if (error && !data) return <><Typography.Title level={3}>仪表盘</Typography.Title><LoadError message={error} onRetry={retry} /></>;
